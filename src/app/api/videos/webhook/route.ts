...
import { db } from "@/db";
import { serviceLogs, videos } from "@/db/schema";
...
export const POST = async (req: Request) => {
  ...
  try {
    ...
    if (err instanceof Error) {
      ...
      // If it's not an Error object, log it as-is
      errorDetails = {
        message: String(err),
      };
    }
    await db.insert(serviceLogs).values({
      service_name: "MUX",
      request_body: payload ? payload : "Could not parse request body",
      response_body: {
        status: "Oops, an error occured in the webhook",
        error: errorDetails,
      },
    });
